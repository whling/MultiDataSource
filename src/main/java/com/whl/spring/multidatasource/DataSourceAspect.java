...
public class DataSourceAspect {
	...
	protected boolean isMatch(String methodName, String mappedName) {
		boolean b = PatternMatchUtils.simpleMatch(mappedName, methodName);
		return b;
	}

	/**
	 * 用户指定slave的方法名前缀,通过set方式注入
	 * 
	 ...
	 */
	/*
	 * public void before(JoinPoint point) { // 获取到当前执行的方法名 String methodName =
	 * point.getSignature().getName(); if (isSlave(methodName)) { // 标记为读库
	 * DynamicDataSourceHolder.markSlave(); } else { // 标记为写库
	 * DynamicDataSourceHolder.markMaster(); } }
	 * 
	 *//**
		 * 判断是否为读库
		 ...
		 *//*
		 * private Boolean isSlave(String methodName) { //
		 * 方法名以query、find、get开头的方法名走从库 return
		 * StringUtils.startsWithAny(methodName, "query", "find", "get"); }
		 */
}
